...
q_in=[0.65*q(:,1)]; q_in=irwsm(q_in,1,0.001); q_in=resample(q_in,6,1); 
%v6=[40*ones(8640,1);]; r6=ftdcurve(p6,v6,2);% v6=[10*ones(100,1); 0.8*v6(2160:4320)];
v6=irwsm(v(:,7),1,0.001); v6=resample(v6,6,1);
v6=[110*ones(100,1); v6(2000:4250)];
...
TTT_open = (sum(q_ramp(start(1):finish)+q2(start(1):finish)-q5(start(1):finish)) + q2(start(1)-1)+q_ramp(start(1)-1)-q5(start(1)-1) ) * (finish-start(1))/3600;
TWT_open = abs(sum(q_urban(start(1):1035)-q_ramp(start(1):1035)))*(1035-start(1))/3600;
TTS_open=TTT_open+TWT_open;

finish=1500;
% -------------- Section 3 Conrol Implementation -------------
uc=[zeros(400,1); (p4(4))*ones(1800,1)];  figure(3); subplot(211); plot(uc,'r'); 
...
Q=eye(size(F)); Q(4,4)=10000; Q(3,3)=1000; Q(2,2)=1; Q(1,1)=1; 
...
for n=3:length(r2);   
    ...
    if  (n>=horizon(1)) & (n<horizon(end))      
        if (i<time+1)&(n==horizon(i))    
           ...
           if q_ramp(n)<0; q_ramp(n)=0; elseif q_ramp(n)>21; q_ramp(n)=21;  end  
    ...
    if  n>=horizon(time)  
        if N_ramp(n)>100; q_ramp(n)=21; %Release cars at capacity level
...
figure(3); 
subplot(211);h1=plot(r4,'g'); 
subplot(212); h1=plot(q_ramp,''); %h5=legend('Urban Demand', 'Unmetered Ramp Flow', 'Metered Ramp Flow');


TTT_llm = (sum(q_ramp(start:finish)+q2(start:finish)-q5(start:finish)) + q2(start-1)+q_ramp(start-1)-q5(start-1) ) * (finish-start)/3600;
...
% figure(11); plot(q5,'k');
% plot(start*ones(100,1),linspace(0,70,100),'r');
% plot(finish*ones(100,1),linspace(0,70,100),'r');
...
% 
effort_llm=sum(q_ramp(start:finish))
% 
% figure(6);
...
% 
% figure(11); 
% subplot(221);plot(q2,'k');
% subplot(222);plot(q3,'k');
% subplot(223);plot(q4,'k');
% subplot(224);plot(q5,'k');
% 

uc=[zeros(200,1); (p4(4))*ones(2000,1)];  %figure(3); subplot(221); plot(uc,'r'); 
...
for n=3:length(r2);   
    ...
    %link 4 [s+1]
    q4(n)=alpha4*min(v4(n-1),v5(n-1))*r4(n-1);%if (n>900)&(n<940); r4(n-1)=r4(n-1)+1;end
    r4(n)=r4(n-1)+((q3(n-1)-q4(n-1))*k4); 
    v4(n)=ftdcurve(p4,r4(n),1);  
    ...
    if  n>=horizon(time)  
        if N_ramp(n)>50; q_ramp(n)=21; %Release cars at capacity level
        elseif (N_ramp(n)<=50)&(N_ramp(n)>=0); q_ramp(n)=0.5*N_ramp(n); if q_ramp(n)>21; q_ramp(n)=21; end   
        elseif N_ramp(n)<10; q_ramp(n)=q_urban(n);   if q_ramp(n)>21; q_ramp(n)=21; end 
        end
    end
    waitbar(n/length(q1),h);
...
figure(3); 
subplot(211);h1=plot(r4,'m'); 
subplot(212);plot(q_ramp,'m');
effort_alinea=sum(q_ramp(start:finish))

TTT_alinea = (sum(q_ramp(start:finish)+q2(start:finish)-q5(start:finish)) + q2(start-1)+q_ramp(start-1)-q5(start-1) ) * (finish-start)/3600;
if TTT_alinea<0; TTT_alinea=0; end
TWT_alinea = (sum(q_urban(start:finish)-q_ramp(start:finish)))*(finish-start)/3600;
TTS_alinea=TTT_alinea+TWT_alinea;


% figure(11); 
% subplot(221);plot(q2,'m');
% subplot(222);plot(q3,'m');
% subplot(223);plot(q4,'m');
% subplot(224);plot(q5,'m');
